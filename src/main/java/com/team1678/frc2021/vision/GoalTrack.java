...
public class GoalTrack {
     ...
     */
    public boolean tryUpdate(double timestamp, Translation2d new_observation, Rotation2d new_observation1) {
        if (!isAlive()) {
            return false;
        }
        double distance = mSmoothedPosition.unaryMinus().rotateBy(new_observation1).getNorm();
    ...
    public void forceUpdate(double timestamp, Translation2d new_observation){
        if(mSmoothedPosition.getDistance(new_observation) > Constants.kMaxTrackerDistance){
